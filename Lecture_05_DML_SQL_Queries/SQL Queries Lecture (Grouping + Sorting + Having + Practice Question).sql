...
-- 1. Find the top five batsman in IPL

SELECT batsman, SUM(batsman_runs) AS 'runs'
FROM ipl.ipl_delivery
GROUP BY batsman
ORDER BY runs DESC LIMIT 5


-- 2. find the 2nd higest 6 hitter in IPL

SELECT batsman, COUNT(*) AS 'num_sixes'
FROM ipl.ipl_delivery
...
-- 3. find the virat kohlis performance against all ipl teams
-- 4. find the top 10 batsman with centuries in IPL

SELECT batsman,match_id,SUM(batsman_runs) AS 'score'
FROM ipl.ipl_delivery
GROUP BY batsman, match_id
Having score >=100
ORDER BY batsman DESC 

-- 5. find the top five batsman with higest strike rate who have played a min of 1000 balls

SELECT batsman, SUM(batsman_runs), COUNT(batsman_runs),
ROUND((SUM(batsman_runs)/COUNT(batsman_runs))* 100,2) AS 'sr'
FROM ipl.ipl_delivery
GROUP BY batsman
